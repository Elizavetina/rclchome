...
document.addEventListener('DOMContentLoaded', function () {
    ...
    document.addEventListener('mousemove', function (event) {
        if (event.target && event.target.tagName.toLowerCase() === 'a') {
            ...
            nLeft = bounds.left + bounds.width / 10;
            nTop = bounds.top + bounds.height / 10;

            rd.classList.add('hovered');
            hrd.classList.add('hovered');

        } else {
            nLeft = event.clientX;
            nTop = event.clientY;
            rd.classList.remove('hovered');
            hrd.classList.remove('hovered');
        }
    })
